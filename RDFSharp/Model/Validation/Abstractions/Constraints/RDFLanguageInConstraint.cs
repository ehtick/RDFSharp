    ...
    public class RDFLanguageInConstraint : RDFConstraint
    {
        ...
        public RDFLanguageInConstraint(List<string> languageTags)
        {
            ...
            languageTags?.ForEach(lt =>
            {
                ...
                if (languageTag == string.Empty || languageTag == "*" || RDFModelShims.LanguageTagRegexShim.Value.Match(languageTag).Success)
                    ...
                    case RDFPlainLiteral valueNodePlainLiteral:
                        ...
                        while (langTagsEnumerator.MoveNext() && !langMatches)
                        {
                            switch (langTagsEnumerator.Current)
                            {
                                ...
                                case "":
                                    langMatches = !Regex.IsMatch(valueNodePlainLiteral.ToString(), string.Concat("@", RDFModelShims.LanguageTagRegexMask, "$"), RegexOptions.Compiled | RegexOptions.IgnoreCase);
                                    break;
                                ...
                                default:
                                    langMatches = Regex.IsMatch(valueNodePlainLiteral.ToString(), string.Concat("@", langTagsEnumerator.Current, RDFModelShims.SubLanguageTagRegexMask, "$"), RegexOptions.IgnoreCase);
                                    break;
                            }
                        }
                        if (!langMatches)
